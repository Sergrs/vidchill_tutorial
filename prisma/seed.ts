...
const videoEngagements = JSON.parse(
...
const announcementEngagements = JSON.parse(
...
async function main() {
  ...
  await processInChunks(followEngagements, 1, async (followEngagement) => {
    ...
    if (existingFollowEngagements.length === 0 || !existingFollowEngagements) {
      return prisma.followEngagement.create({
        data: followEngagement,
      });
    } else {
      return;
    }
